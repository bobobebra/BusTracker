...
export default async function handler(req, res) {
  try {
    ...
    // Map: shape_id -> [ [lat, lon], ... ] (preserve sequence)
    ...
    for (const [sid, arr] of shapePoints) {
      arr.sort((a, b) => a.seq - b.seq);
    }

    // Map: route_id -> set(shape_id) using trips.txt
    ...
    // Build GeoJSON features per route (MultiLineString)
    const features = [];
    ...
    return res.status(500).json({ error: "Failed to build shapes" });
  }
}
